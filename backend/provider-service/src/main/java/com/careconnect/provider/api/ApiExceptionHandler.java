...
public class ApiExceptionHandler {
    ...
    ProblemDetail onNotFound(DoctorNotFoundException ex) {
        ProblemDetail problem = ProblemDetail.forStatus(HttpStatus.NOT_FOUND);
        problem.setType(URI.create(ERROR_NS + "doctor-not-found"));
        problem.setTitle("Doctor not found");
        problem.setDetail(ex.getMessage());
        return problem;
    }

    @ExceptionHandler(com.careconnect.provider.domain.OverlappingSlotException.class)
    ProblemDetail onOverlap(com.careconnect.provider.domain.OverlappingSlotException ex) {
        ProblemDetail problem = ProblemDetail.forStatus(HttpStatus.CONFLICT);
        problem.setType(URI.create(ERROR_NS + "availability-conflict"));
        problem.setTitle("Availability conflict");
        problem.setDetail(ex.getMessage());
        return problem;
    }
