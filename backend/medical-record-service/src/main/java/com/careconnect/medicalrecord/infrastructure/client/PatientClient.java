 ...
 * we ask, we never command). Encounters are keyed by patient-service's
 * patient id, not the identity user id, so "my records" must resolve through
 * here rather than treating the JWT subject as the patient id directly.
 */
