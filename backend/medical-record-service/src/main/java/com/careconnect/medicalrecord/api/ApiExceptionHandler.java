...
public class ApiExceptionHandler {
    ...
    ProblemDetail onNotFound(EncounterNotFoundException ex) {
        ProblemDetail problem = ProblemDetail.forStatus(HttpStatus.NOT_FOUND);
        problem.setType(URI.create(ERROR_NS + "encounter-not-found"));
    ...
    ProblemDetail onIllegalState(IllegalStateException ex) {
        // e.g. editing a SIGNED encounter, or signing without notes
        ProblemDetail problem = ProblemDetail.forStatus(HttpStatus.CONFLICT);
        problem.setType(URI.create(ERROR_NS + "record-state"));
        problem.setTitle("Record state conflict");
        problem.setDetail(ex.getMessage());
        return problem;
    }
