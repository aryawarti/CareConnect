...
public class ApiExceptionHandler {
    ...
    ProblemDetail onNotFound(PatientNotFoundException ex) {
        ProblemDetail problem = ProblemDetail.forStatus(HttpStatus.NOT_FOUND);
        problem.setType(URI.create(ERROR_NS + "patient-not-found"));
        problem.setTitle("Patient not found");
        problem.setDetail(ex.getMessage());
        return problem;
    }

    @ExceptionHandler(com.careconnect.patient.domain.ProfileAlreadyExistsException.class)
    ProblemDetail onProfileExists(com.careconnect.patient.domain.ProfileAlreadyExistsException ex) {
        ProblemDetail problem = ProblemDetail.forStatus(HttpStatus.CONFLICT);
        problem.setType(URI.create(ERROR_NS + "profile-exists"));
