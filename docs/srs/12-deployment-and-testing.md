...
|---|---|---|---|
| Unit | Domain rules in isolation — state machines, fairness ordering, money arithmetic, wait estimation | JUnit 5, AssertJ, Mockito | Built |
| Slice | Controller contracts, validation, authorization (401/403 paths) | `@WebMvcTest`, MockMvc | Built |
| Data | Repositories and migrations against **real PostgreSQL** | `@DataJpaTest` + Testcontainers | Built |
| Integration | Full context: REST in → DB + events out | `@SpringBootTest` + Testcontainers (Postgres, Kafka) | Built |
| Contract | Feign client ↔ provider API compatibility | WireMock stubs | Built |
| End-to-end | Cross-service journeys through the gateway | Compose + REST assertions | Partial |
| UI | Component and journey tests | Jasmine/Karma; Playwright *(Planned)* | Planned |
...
| Unrelated doctor reads a chart | 403 despite a valid DOCTOR role — relationship gating works |
